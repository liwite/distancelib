	...
	$.fn.smartMenu = function(data, options) {
							...
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li" data-hover="true">' + htmlCreateMenu(obj.data) +
										'<a href="javasc	ript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"><i class="'+ clKey +'triangle"></i>'+ text +'</a>' + 
									'</li>';
								} else {
				...
				$(idKey + params.name +" li").each(function() {
					...
					$(this).hover(function() {
						...
						if (isHover) {
							...
							var top=$('.smart_menu_li_hover '+'.'+ clKey +'box' ).height();
							$('.smart_menu_li_hover '+'.'+ clKey +'box' ).css({
								top:0
							});
							if(e.pageY>_screen_height*0.5){
								$('.smart_menu_li_hover '+'.'+ clKey +'box' ).css({
									top:-(top-25)
								});
							}
		...
		$(this).each(function() {
			this.oncontextmenu = function(e) {
				...
				$.smartMenu.hide();
				var st = D.scrollTop(),
				 jqueryMenu = funSmartMenu(),
				 left=e.pageX,top= e.pageY,
				 menu_height=jqueryMenu.height();
				if( e.pageY>_screen_height*0.5){
