    ...
    export class CellFactory {
        ...
        private _getGroupHeader(gr: GroupRow): string {
            ...
            if (group && fmt) {
                ...
                var isHtml = gr.isContentHtml; // TFS 114902
                if (col) {
        ...
        constructor(grid: any, pt: any) {
            ...
            if (this._p != null) {
                ...
                if (this._col > -1) {
                    ...
                    if (pt.x - col.pos - pt.x <= szEdge) this._edge |= 1; // left
                    if (col.pos + col.renderSize - pt.x <= szEdge) this._edge |= 4; // right
                }
                if (this._row > -1) {
                    var row = rows[this._row];
                    if (pt.y - row.pos - pt.y <= szEdge) this._edge |= 2; // top
...
module wijmo.grid {
    ...
    export class _MouseHandler {
        ...
        private _mouseUp(e: MouseEvent) {
            ...
            if (htd && htd.cellType == CellType.TopLeft && htd.row == 0 && htd.col == 0 && !this._szArgs) {
                var g = this._g,
                    ht = g.hitTest(e);
